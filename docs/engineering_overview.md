...
The production write path is complete. It extracts DOCX
...
preflights a replacement plan without printing raw values. It then applies the
plan, saves atomically, reopens and structurally validates the DOCX, and scans
the supported output text for residual high-confidence detections.
...
[the Phase 1 findings](phase1_findings.md).
...
ephemeral in-process key. A write-enabled run requires an explicit secret and
output path:

```bash
export PII_REDACTION_KEY='private-secret-of-at-least-16-bytes'
...
[here](phase2c_holdout_initial_75ce5f8.json).

After development-driven semantic repairs and the bounded PERSON pass, the old
holdout's latest non-blind diagnostic has micro precision 0.843, recall 0.901,
and F1 0.871. COMPANY precision improved from 0.250 to 0.909; PERSON precision
remains a concern at 0.674. These are diagnostic rather than blind final metrics.
...
preflights all affected runs and text-box mirrors. The delivered prospectus copy
contains 648 applied replacements, including 38 same-container exact-repeat
propagations and 380 cross-run spans, with zero structural conflicts.
...
- The redacted prospectus was saved, reopened, structurally compared, and
  scanned. At confidence >=0.85, supported text has no wholly unclassified
  residual; four broad detections overlapping synthetic spans require review.
  Realistic synthetic values remain intentionally PII-shaped.
- Raster-image OCR and automatic rewriting of shape metadata remain out of
  scope, so zero total-document leakage is not claimed.
